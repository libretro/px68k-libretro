...
void FASTCALL PIA_Write(DWORD adr, BYTE data)
{
	...
	if ( adr==0xe9a005 )
   {
      ...
      if ( (portc&0x10)!=(pia.PortC&0x10) )
         Joystick_Write(0, (BYTE)((data&0x10)?0xff:0x00));
      if ( (portc&0x20)!=(pia.PortC&0x20) )
         Joystick_Write(1, (BYTE)((data&0x20)?0xff:0x00));
   }
   else if ( adr==0xe9a007 )
   {
      if ( !(data&0x80) )
      {
         ...
         if ( (portc&0x0f)!=(pia.PortC&0x0f) )
            ADPCM_SetPan(pia.PortC&0x0f);
         if ( (portc&0x10)!=(pia.PortC&0x10) )
            Joystick_Write(0, (BYTE)((data&1)?0xff:0x00));
         if ( (portc&0x20)!=(pia.PortC&0x20) )
            Joystick_Write(1, (BYTE)((data&1)?0xff:0x00));
      }
   }
